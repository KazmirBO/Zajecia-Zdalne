...
void Wprowadzanie(int tab[], int N){
  for(int i=N;i>0;i--) tab[N-i]=i;
}
void Wprowadzanie2(int tab[], int N){
  for(int i=0;i<N;i++) tab[i]=i+1;
  if(N==10) std::swap(tab[2],tab[7]);
  else std::swap(tab[250],tab[750]);
}
...
void Tmp(int tab[], int tab_t[], int N){
  for(int i=0;i<N;i++) tab_t[i]=tab[i];
}
...
void MergeSort(int tab[], int startIndex, int middleIndex, int endIndex){
	...
	for(int i=0;i<totalElements;++i) tab[startIndex+i]=tempArray[i];
	delete []tempArray;
}
