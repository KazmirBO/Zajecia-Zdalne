...
Data Data::operator+(const Data &b) const{
  return Data(Rok+b.Rok, Mies+b.Mies, Dzien+b.Dzien, Godz+b.Godz, Min+b.Min);
}
Data::Data(){
  Rok=Mies=Dzien=Godz=Min=0;
}
Data Data::operator-(const Data &b) const{
  return Data(Rok-b.Rok, Mies-b.Mies, Dzien-b.Dzien, Godz-b.Godz, Min-b.Min);
}
std::ostream &operator<<(std::ostream &os, const Data &v){
  os<<"Data: "<<v.Rok<<"-"<<v.Mies<<"-"<<v.Dzien<<" "<<v.Godz<<":"<<v.Min;
  return os;
}
void Data::Dodawanie(){
  if(Min>=60){
