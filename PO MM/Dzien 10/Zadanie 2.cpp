...
void Transport::f_skladowa(){
  ...
  rezultat=count_if(p,k,[this](int j){skl_c++;return(j<(skl_a+this->skl_b));});
  std::cout<<"Cialo lambdy wykorzystalo skladniki na 2 sposoby. "<<std::endl;
  ...
  rezultat=count_if(p,k,[=](int j){return(j<(2*skl_a+this->skl_b));});
