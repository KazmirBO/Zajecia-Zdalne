...
void Postac::GetStat(int N, int D){
  HP=N+rand()%D+1; //N - Domyslna wartosc zdrowia, D - Dodatkowy rzut na zdrowie
...
int Postac::Atak(int Spell){
  ...
  if(rand()%11==10){ // Jezeli kosc d10 == 10 dodatkowa kosc obrazen
    int kryt=rand()%6+1;
    std::cout<<"Trafienie Krytyczne! Dodatkowe: "<<kryt<<" obrazen."<<std::endl;
    Rezultat+=kryt;
  }
...
int Postac::Czary(){
  if(Mana>=10){
    ...
    std::cout<<"Bohater uzyl czaru zadajac: "<<czar<<" obrazen."<<std::endl;
    return czar;
  }
