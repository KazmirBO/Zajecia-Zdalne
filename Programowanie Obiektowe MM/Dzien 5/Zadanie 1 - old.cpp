...
public:
  ...
  friend bool SprCzy(Pionek Pion, Pionek Bialy[], Pionek Czarny[],
    int IleB, int IleC);
  friend int poza(Pionek &p, Kwadrat &k);
  friend void Graphic(Pionek Bialy[], Pionek Czarny[],
    Kwadrat Plansza, int IleB, int IleC);
};
class Kwadrat{
  int x, y;
  int bok;
...
int poza(Pionek &pt, Kwadrat &kw){
  ...
  if(((pt.x>kw.x)&&(pt.x<(kw.x+kw.bok)))&&((pt.y>kw.y)&&(pt.y<(kw.y+kw.bok)))){
    return 1;
  }
  else std::cout<<"Pionek: "<<pt.nazwa<<" wypadl poza "<<kw.nazwa<<std::endl;
  return 0;
}
void Graphic(Pionek Bialy[], Pionek Czarny[], Kwadrat Plansza,
  int IleC, int IleB);
...
int main(){
  ...
  if(Wygral) std::cout<<"Wygral gracz grajacy Czarnymi pionkami."<<std::endl;
  else std::cout<<"Wygral gracz grajacy Bialymi pionkami."<<std::endl;
  return 0;
}
