...
int main(){
  ...
  switch (znak) {
    ...
    case '-':
      ...
      break;
    case '*':
      printf("Mnożenie liczb z1*z2 wynosi: %.2f+i%.2f", (a*c)-(b*d),(a*d)+(b*c));
      break;
    case '/':
      if((c*c)+(d*d)>0) printf("Dzielenie liczb z1/z2 wynosi: %.2f+i%.2f", ((a*c)+(b*d))/((c*c)+(d*d)),((b*c)-(a*d))/((c*c)+(d*d)));
      else printf("Podano bledne liczby.");
      break;
    default:
